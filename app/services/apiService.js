...
const API_BASE_URL = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:8080/api/v1/';
...
export const getEligibleOffers = async (token, amount) => {
    ...
    try {
        ...
        console.warn(`Live API call to getEligibleOffers failed: ${error.message}. Returning mock data as a fallback.`);
...
export const getApplicationHistory = async (token) => {
    ...
    try {
        return await fetchWithAuth('applications/my-applications', token, { method: 'GET' });
    } catch (error) {
        console.warn(`Live API call to getApplicationHistory failed: ${error.message}. Returning mock data as a fallback.`);
...
export const startApplication = async (token, applicationData) => {
    if (USE_MOCK_DATA_ALWAYS) {
        ...
        return Promise.resolve({ applicationId: Math.floor(Math.random() * 10000), uuid: crypto?.randomUUID?.() || 'mock-uuid-app' })
    }
...
export const getHospitals = async (token, hospitalId = null) => {
    ...
    if (USE_MOCK_DATA_ALWAYS) {
        const hospitals = [
            { id: 101, name: 'Apollo Hospitals', location: 'Chennai, Tamil Nadu', code: 'H001', type: 'Multi-Specialty Hospital' },
            { id: 102, name: 'Fortis Healthcare', location: 'Mumbai, Maharashtra', code: 'H002', type: 'Super Specialty Hospital' },
            { id: 103, name: 'Max Healthcare', location: 'Delhi NCR', code: 'H003', type: 'Multi-Specialty Hospital' },
        ];
    ...
    return await fetchWithAuth(`master/hospital${query}`, token, { method: 'GET' });
}
 ...
 */
export const selectEmiForApplication = async (token, applicationId, emiPlanId) => {
    ...
    return await fetchWithAuth(`applications/${applicationId}/select-emi/${emiPlanId}`, token, {
        method: 'POST',
...
export const getEmiPlans = async (token, applicationId) => {
    ...
    // This will create a URL like: /api/v1/master/emi-plans?hospitalId=101
        return await fetchWithAuth(`applications/${applicationId}/emi-plans`, token, { method: 'GET' });
};
 ...
 */
export const submitManualApplication = async (token, applicationId, manualData) => {
    ...
    // This calls your new backend endpoint.
    return await fetchWithAuth(`applications/${applicationId}/submit-manual`,
        token, {
        method: 'POST',
